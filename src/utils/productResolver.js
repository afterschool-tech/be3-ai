 ...
 * @param {object} [constraints] - Optional search constraints (category, brand, etc.)
 * @returns {Promise<string|null>} - The resolved canonical ID or null.
 */
async function resolveProduct(identifier, context, constraints = {}) {
    ...
    // 3. Fallback: Search by name to find the best ID match (WITH CONSTRAINTS)
    try {
        const queryParams = {
            q: identifier,
            per_page: '1'
        ...
        if (constraints.category) queryParams.category = constraints.category;
        if (constraints.brand) queryParams.brand = constraints.brand;
        if (constraints.vendor) queryParams.vendor = constraints.vendor;
        ...
        if (products.length > 0) {
            ...
            console.log(`[ProductResolver] Resolved "${identifier}" via search fallback (constraints: ${JSON.stringify(constraints)}): ${resolvedId}`);
            return resolvedId;
        }
    } catch (e) {
